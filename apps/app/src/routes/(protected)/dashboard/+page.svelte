	...
	<main class="mx-auto max-w-5xl px-4 py-10">
		<div class="rounded-2xl bg-white p-8 shadow-sm">
			<h2 class="text-2xl font-bold text-gray-900">
				{m.dashboard_welcome({ name: data.user.name })}
			</h2>
			<p class="mt-2 text-gray-500">{m.dashboard_signed_in_with_google()}</p>

			<dl class="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
				<div class="rounded-lg bg-gray-50 p-4">
					<dt class="text-xs font-medium uppercase tracking-wide text-gray-400">{m.dashboard_name_label()}</dt>
					<dd class="mt-1 text-sm font-medium text-gray-900">{data.user.name}</dd>
				</div>
				<div class="rounded-lg bg-gray-50 p-4">
					<dt class="text-xs font-medium uppercase tracking-wide text-gray-400">{m.dashboard_email_label()}</dt>
			...
			<div class="mt-8 grid grid-cols-1 gap-4 md:grid-cols-2">
				...
				<section class="flex flex-col justify-between rounded-2xl bg-gray-50 p-5 shadow-sm">
					<div class="space-y-2 text-left">
						<p class="text-xs font-semibold uppercase tracking-wide text-gray-500">{m.dashboard_subscription_label()}</p>
						{#if data.subscription}
							{#if data.subscription.cancelAtPeriodEnd}
								<p class="text-sm font-medium text-gray-900">{m.dashboard_canceling_status()}</p>
								<p class="text-xs text-gray-500">
									{m.dashboard_canceling_detail({ date: new Date(data.subscription.currentPeriodEnd).toLocaleDateString() })}
								</p>
							{:else}
								<p class="text-sm font-medium text-gray-900">{m.dashboard_active_status()}</p>
								<p class="text-xs text-gray-500">
									{m.dashboard_active_detail({ date: new Date(data.subscription.currentPeriodEnd).toLocaleDateString() })}
								</p>
							{/if}
						{:else}
							<p class="text-sm font-medium text-gray-900">{m.dashboard_free_status()}</p>
							<p class="text-xs text-gray-500">{m.dashboard_free_detail()}</p>
						{/if}
					</div>
						...
						{:else if !data.subscription.cancelAtPeriodEnd}
							{#if isConfirmingCancel}
								...
								>
									...
									>
										{isCanceling ? m.dashboard_canceling_button() : m.dashboard_confirm_cancel()}
									</button>
								</form>
								<button
							...
							{:else}
								...
								>
									{m.dashboard_cancel_subscription()}
								</button>
							{/if}
						{/if}
					</div>
				...
				<section class="flex flex-col justify-between rounded-2xl bg-red-50 p-5 shadow-sm">
					<div class="space-y-2 text-left">
						<h3 class="text-xs font-semibold uppercase tracking-wide text-red-700">{m.dashboard_danger_zone()}</h3>
						<p class="text-sm font-medium text-red-900">{m.dashboard_delete_account_heading()}</p>
						<p class="text-xs text-red-700">
							{m.dashboard_delete_account_body()}
						</p>
					</div>
					...
					{#if isConfirmingDelete}
						<div class="mt-4 flex flex-wrap items-center gap-3">
							...
							>
								...
								>
									{isDeleting ? m.dashboard_deleting_button() : m.dashboard_confirm_delete()}
								</button>
							</form>
							<button
					...
					{:else}
						...
						>
							{m.dashboard_delete_account_button()}
						</button>
					{/if}
				</section>
			</div>
