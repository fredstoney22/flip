...
`apps/app` uses **Paraglide JS** (`@inlang/paraglide-js`) for i18n. Target locales: `en` (base) and `es-419` (Latin American Spanish, neutral — not `es-ES`). As of this writing only the `en` message catalog is populated; `es-419` translations are a separate follow-up.

- **Message source**: `apps/app/messages/en.json` (inlang message format — `{param}` placeholders). Compiled output lands in `apps/app/src/lib/paraglide/` (gitignored, regenerated by the Vite plugin on every dev/build run — never hand-edit it).
