...
`apps/app` uses **Paraglide JS** (`@inlang/paraglide-js`) for i18n. Target locales: `en` (base) and `es-419` (Latin American Spanish, neutral — not `es-ES`, informal "tú" register). Both `en` and `es-419` message catalogs are fully populated and kept in lockstep — every key in one must exist in the other with matching `{param}` placeholders.

- **Message source**: `apps/app/messages/en.json` and `apps/app/messages/es-419.json` (inlang message format — `{param}` placeholders). Compiled output lands in `apps/app/src/lib/paraglide/` (gitignored, regenerated by the Vite plugin on every dev/build run — never hand-edit it).
- **Config**: `apps/app/project.inlang/settings.json` (locales, base locale, message-format plugin path pattern) and the `paraglideVitePlugin(...)` block in `apps/app/vite.config.ts` (locale resolution `strategy` array).
- **Usage in components/routes**: `import * as m from '$lib/paraglide/messages'`, then call `m.some_key()` (or `m.some_key({ param })` for parameterized messages).
- **Locale resolution** (`apps/app/src/hooks.server.ts`, via `paraglideMiddleware`): an explicit user choice — mirrored into the `PARAGLIDE_LOCALE` cookie by `settings.setLocale()` in `apps/app/src/lib/stores/settings.ts` — wins; otherwise the `Accept-Language` header is used; final fallback is `en`. `apps/app/src/app.html`'s `<html lang="%paraglide.lang%">` placeholder is substituted by the same middleware. There is no locale-switcher UI yet (planned follow-up) — locale is set via the cookie/header only.
- **Constants that build translated data** (e.g. `tutorialSteps.ts`'s `getDefaultTutorial()`, `firstStepsTutorials.ts`'s `getFirstStepsTutorials()`) are **functions, not module-level constants** — they must be called fresh per request/render so `m.*()` calls resolve the current request's locale via Paraglide's `AsyncLocalStorage`-backed `getLocale()`, rather than freezing whatever locale was active when the server process first imported the module.
- **`packages/game` stays framework-agnostic** — it has no Paraglide dependency. English-only labels that live there (`FIRST_STEPS_CONCEPTS` in `packages/game/src/puzzles/firstSteps.ts`, pack `name` fields in `packGenerationSpecs.ts`) are overridden by slug/id-keyed lookups in `apps/app` (`src/lib/constants/firstStepsConcepts.ts`, `src/lib/constants/packNames.ts`), the same pattern `packDescriptions.ts` already used for pack copy. Any route that displays a pack's raw name from the API response (e.g. `puzzleData.packName`) must run it through `getPackName(slug) ?? rawName` before rendering — see `+page.server.ts` in `routes/(game)/play`, `routes/(game)/play/game`, `routes/(game)/play/puzzles`, and `routes/tutorial` for the pattern. `formatLongDate` in `src/lib/utils/date.ts` is hardcoded to `en-US` formatting regardless of locale (`formatShortDate` uses the runtime's default locale instead, but is equally unaware of the user's chosen Paraglide locale) — a known, not-yet-addressed gap (date display, not message-catalog copy).
- **Out of scope by design**: `routes/privacy` and `routes/terms` stay English-only (legal copy, not translated), and the dev-only `puzzle-lab`/debug routes are not extracted.
